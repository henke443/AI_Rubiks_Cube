...
class RubiksEnv(gym.Env):
    ...
    def __init__(self, moves_per_step=1, terminate_after_n_moves: int | str = False,
        ...
        elif self._obs_dim == "2d":
            self.observation_space = spaces.Box(
                0, 53, shape=(6, 9), dtype=np.int8)

        elif self._obs_dim == "3d":
            self.observation_space = spaces.Box(
    ...
    def _get_multi_dim_obs(self):
        ...
        for cube_face_i in range(0, 6):
            ...
            for face_row in range(0, 3):

                # cubie_face = np.array([])
                # for row_i in range(0, 3):
                discrete_row = np.array(
                    self.cube.get_strip(
                        cube_face_i, "row", face_row
                    ), dtype=np.int8)

                for row_i, row_el in enumerate(discrete_row):
                    # print("cubie_face_i:", face_row,  row_el)

                    if self._obs_dim == "3d":
                        bin_cubie_face = np.zeros(
                            shape=(53 if full_info else 6,), dtype=np.bool_)
                        bin_cubie_face[row_el if full_info else cube_face_i] = 1
                        # cubie_face = bin_cubie_face
                        retVal[cube_face_i][3*face_row+row_i] = bin_cubie_face
                    else:
                        retVal[cube_face_i][3*face_row +
                                            row_i] = row_el if full_info else cube_face_i
    ...
    def _get_obs(self):
        return self._get_flat_obs() if self._obs_dim == "flat" else self._get_multi_dim_obs()
    ...
    def reset(self, fixed_extra_scrambles=None):
        ...
        )["distance"] if self._obs_dim == "flat" else 1
